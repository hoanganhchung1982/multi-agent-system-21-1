...
export const Layout: React.FC<LayoutProps> = React.memo(({ children, onBack, title }) => {
  return (
    <div className="min-h-screen flex flex-col bg-white text-slate-900">
        ...
        <div className="space-y-1">
          ...
          {title && (
            <div className="pt-4 flex justify-center">
              <span className="px-4 py-1 bg-indigo-50 text-indigo-700 rounded-full text-sm font-bold tracking-wide uppercase shadow-sm">
                {title}
              </span>
            </div>
          )}
      ...
      {/* Footer chuẩn hóa Gian hàng Chuyển đổi số */}
      <footer className="py-10 px-6 text-center mt-8">
